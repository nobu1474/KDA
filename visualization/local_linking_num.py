...
from core.open_curve_jones import open_curve_Linking_num
...
def plot_local_linking_num_by_dimension(
    birth_death_pairs,
    ...
    for col_idx, dim in enumerate(dims_to_plot, start=1):
        ...
        weights = []
        w = 1.0
        for p in tqdm(pairs, desc=f"Linking num {dim}"):
            ...
            Linking_num = open_curve_Linking_num(polys) if polys else 0 
            
            weights.append(Linking_num)
        ...
        if plot_type == "barcode":
            ...
            for i, (p, w) in enumerate(sorted_pairs_and_weights):
                b = float(p["birth"])
                if p["death"] is None:
                    d = infinite_y
                    is_infinite = True
