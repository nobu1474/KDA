...
# 5. 全体：交点検出関数
def find_crossings(curves, projection_vector=np.array([0, 0, 1]), closed=True):
    crossings = []

    projected_curves = [
        project_to_2D(curve, projection_vector)
        for curve in curves
    ]

    # 線分リスト
    # closed=True の場合、各 curve はすでに curve[-1] == curve[0] を含む想定。
    # したがって最後と最初をここでは結ばない。
    segments = []
    ...
    # 各成分の線分数。closed=True なら len(curve)-1。
    curve_segment_counts = np.array([
        max(len(curve) - 1, 0)
        for curve in curves
    ])
    ...
    # 同じ成分上で隣り合う線分は除外。
    # closed=True の場合、最初の線分と最後の線分も同じ頂点を共有するので除外。
    adjacent = same_curve & (diff <= 1)

    if closed:
        n_segs1 = curve_segment_counts[ci1]
        adjacent |= same_curve & (diff == n_segs1 - 1)
    ...
    p_cross = p1_i3d + t_intersect[:, np.newaxis] * dp_i3d

    q1_i3d = p1[j_intersect]
    dq_i3d = p2[j_intersect] - p1[j_intersect]
    q_cross = q1_i3d + s_intersect[:, np.newaxis] * dq_i3d

    p_depth = np.dot(p_cross, projection_vector)
    q_depth = np.dot(q_cross, projection_vector)

    p_over = p_depth > q_depth
    ...
    for k in range(len(i_intersect)):
        crossings.append({
            ...
            "curves": (
                int(ci_arr[i_intersect[k]]),
                int(ci_arr[j_intersect[k]]),
            ),
...
def count_state_cycles_by_orbits(crossings, state, n_curves=1): # なぜcrossings, stateだけで計算できる？
    ...
    for curve_events in ordered_events:
        ...
        for i, (crossing_index, branch_index) in enumerate(curve_events):
            next_crossing_index, next_branch_index = curve_events[(i + 1) % n_events]
