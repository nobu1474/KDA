...
class Knot:
    ...
    # @property
    # def writhe(self) -> int:
    #     return int(sum(int(crossing["sign"]) for crossing in self.crossings))

    # @property
    # def bracket_polynomial_A(self) -> Polynomial:
    #     self._ensure_bracket()
    #     return self._bracket_A

    # @property
    # def normalized_polynomial_A(self) -> Polynomial:
    #     self._ensure_normalized_A()
...
# TODO: jones多項式を求めるのに必要なものだけにしたい。求めるための関数だけ入れるのでもいいけど、jonoes多項式の計算に使うプロパティだけは入れておく？（ブラケット、ねじれ数、交点数とか）